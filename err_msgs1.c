 ...
 * Return: The error string.
 */
OAchar *error_2_syntax(char **args)
OA{
	char *error, *hist_str;
OAOAOAOA	int len;
OAOA
	hist_str = _itoa(hist);
OAOAOAOAOAOAOA	if (!hist_str)
		return (NULL);

OA	len = _strlen(name) + _strlen(hist_str) + _strlen(args[0]) + 33;
OA	error = malloc(sizeof(char) * (len + 1));
OA	if (!error)
	{
		free(hist_str);
OAOA		return (NULL);
...
OA	_strcat(error, ": ");
	_strcat(error, hist_str);
OAOA	_strcat(error, ": Syntax error: \"");
	_strcat(error, args[0]);
OAOA	_strcat(error, "\" unexpected\n");
OA
OA	free(hist_str);
	return (error);
OA}
